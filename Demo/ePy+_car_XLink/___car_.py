from machine import I2C, LED, PIN, UART
import _thread
import utime
# from machine import ultr
import os
...
def dj_ctr(cmd):
    ...
    motor_ctrl.send(buf1, 0x52)

    buf = [index, angle >> 8, angle & 0xff]
    # motor_ctrl.write(1,82,buf,len(buf),1)
    motor_ctrl.send(buf1, 0x52)


def carforward(speed):
    global motor_ctrl
    # motor_ctrl.write(1,0x52,[0x00,0x00,speed],3,1)
    motor_ctrl.send([0x00, 0x00, speed], 0x52)
    # motor_ctrl.write(1,0x52,[0x02,0x00,speed],3,1)
    motor_ctrl.send([0x02, 0x00, speed], 0x52)


def carback(speed):
    global motor_ctrl
    # motor_ctrl.write(1,0x52,[0x00,0x01,speed],3,1)
    motor_ctrl.send([0x00, 0x01, speed], 0x52)
    # motor_ctrl.write(1,0x52,[0x02,0x01,speed],3,1)
    motor_ctrl.send([0x02, 0x01, speed], 0x52)
...
def carstop():
    global motor_ctrl
    # motor_ctrl.write(1,0x52,[0x00,0x00,0x00],3,1)
    motor_ctrl.send([0x00, 0x00, 0x00], 0x52)
    # motor_ctrl.write(1,0x52,[0x02,0x00,0x00],3,1)
    motor_ctrl.send([0x02, 0x00, 0x00], 0x52)
...
def uploadData():
    ...
    while isRun:
        ...
        if ctrModel != 3:
            pass
            # s = ul.distance()
            # print('<<<Ultrasonic : {0} mm'.format(s))
        utime.sleep(1)
